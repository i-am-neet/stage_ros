...
StageNode::ImportModel(Stg::Model* mod )
{
    ...
    if (dynamic_cast<Stg::ModelRanger *>(mod)) {
        ...
        p->posestamped_sub = n_.subscribe<geometry_msgs::PoseStamped>(mapName(POSESTAMPED, mp), 10, boost::bind(&StageNode::poseStampedReceived, this, position_model_idx, _1));
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname )
{
    ...
    stall_all_pub_ = n_.advertise<stage_ros::Stall>("/stalled_robots", 10);
...
StageNode::WorldCallback()
{
    ...
    stage_ros::Stall stall_robots_msg;
    stall_robots_msg.header.frame_id = "map";
    stall_robots_msg.header.stamp = this->sim_time;
    for (size_t r = 0; r < this->positionmodels.size(); r++) {
        if (this->positionmodels[r]->Stalled()) {
            stall_robots_msg.stalled_robots_num.push_back(r);
        }
    }
    this->stall_all_pub_.publish(stall_robots_msg);
