...
StageNode::SubscribeModels()
{
    ...
    stall_pub_ = n_.advertise<stage_ros::Stall>("stalled_robots", 10);
...
StageNode::WorldCallback()
{
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        stall_robots_msg.header.stamp = sim_time;

        if (robotmodel->positionmodel->Stalled()) {
    ...
    this->stall_pub_.publish(stall_robots_msg);
}
